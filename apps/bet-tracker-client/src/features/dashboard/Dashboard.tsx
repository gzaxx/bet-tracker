...
export const Dashboard = () => {
  ...
  const activePortfolios = activeProfile
    ? portfolios.filter((portfolio) => portfolio.profileId === activeProfile.id)
    : [];
  const selectedPortfolio = activePortfolios.find(
  ...
  const confirmDelete = async () => {
    ...
    if (deleted) {
      if (
        ...
        pendingDelete.id === selectedPortfolioId
      ) {
        setSelectedPortfolioId(null);
      }
  ...
  return (
    <>
      <div>
        ...
        <Title order={1} size="h1" mt={4}>
          Manage your investing workspace
        </Title>
        <Text c="dimmed">
          Create portfolios, review positions, record trades, and keep market
          prices current.
        </Text>
      </div>
      <ErrorBanner />
        ...
        <Group justify="space-between" align="flex-start" wrap="nowrap">
          <div>
            <Badge color="cyan" variant="light" mb="sm">
              {activeProfile.defaultCurrency} workspace
            </Badge>
            <Title order={2} size="h2">
              Stay close to the numbers.
            </Title>
            <Text c="dimmed" maw={560} mt="xs">
              Select a portfolio to review your holdings, record a trade, or
      ...
      >
        <div>
          ...
          <Text size="sm" c="dimmed">
            Every portfolio reports in {activeProfile.defaultCurrency}.
          </Text>
        </div>
      </Group>
      <SimpleGrid cols={{ base: 1, sm: 2, lg: 3 }} mb="md">
        {activePortfolios.map((portfolio) => (
          <PortfolioCard
            ...
            onSelect={(candidate) => setSelectedPortfolioId(candidate.id)}
            onDelete={(candidate) =>
              setPendingDelete({
                kind: "portfolio",
                id: candidate.id,
                label: `portfolio “${candidate.name}”`,
              })
            }
            key={portfolio.id}
          />
            ...
            <Stack gap="sm">
              <Group gap="sm">
                ...
                <div>
                  ...
                  <Text fw={700}>Create a portfolio</Text>
                </div>
              </Group>
              <TextInput
              ...
              <Group justify="space-between" align="center">
                <Text size="xs" c="dimmed">
                  Currency: {activeProfile.defaultCurrency}
                </Text>
                <Button
                  type="submit"
                  size="sm"
      ...
      {activePortfolios.length === 0 && (
        <Paper withBorder radius="lg" p="xl" ta="center" mb="md">
          <ThemeIcon
            variant="light"
            color="indigo"
            size={48}
          ...
          <Text size="sm" c="dimmed">
            Use the card above to create a portfolio and start recording trades.
          </Text>
        </Paper>
      )}
      {selectedPortfolio && (
        <Stack gap="lg">
          ...
          <TradeManager
            portfolio={selectedPortfolio}
            onChanged={() => setSummaryRefreshKey((current) => current + 1)}
          />
          <PriceManager
            portfolio={selectedPortfolio}
            onChanged={() => setSummaryRefreshKey((current) => current + 1)}
