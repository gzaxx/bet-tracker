...
export const Onboarding = () => {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true);
    await createProfileAndPortfolio(
      { name: profileName, defaultCurrency: currency },
      portfolioName,
    );
    setSubmitting(false);
  };
  ...
  return (
    <Container size="sm" py={{ base: 48, sm: 100 }}>
      <Stack gap="xl">
        <div>
          <Badge variant="light" color="indigo" mb="md">
            Welcome to Bet Tracker
          </Badge>
          <Title order={1} size="h1" mb="sm">
            A easy way to track your portfolio.
          </Title>
          <Text size="lg" c="dimmed">
            Create a profile and your first portfolio. Keep trades, prices, and
            performance together in one focused workspace.
          </Text>
        </div>
        {error && (
          <Alert color="red" title="Unable to create workspace">
          ...
          <form onSubmit={submit}>
            <Stack gap="md">
              ...
              <Button
                ...
                size="md"
                loading={submitting}
                leftSection={!submitting && <IconPlus size={17} />}
              >
